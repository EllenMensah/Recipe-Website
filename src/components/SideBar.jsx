...
const Sidebar = () => {
  return (
    <div className="flex flex-col w-60 gap-y-6 bg-white shadow h-screen px-2 py-4 fixed left-0 top-0">
      <span>
        <img src={Fufu2} alt="" className="w-50 h-auto drop-shadow-md" />
      </span>
        ...
        {K.NAVLINKS.map((link, index) => {
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                `text-gray-700 hover:bg-[#FEF2F22qaz] hover:text-black px-3 py-2 w-full rounded-md flex items-center gap-x-2 ${isActive ? "bg-[#FEF2F2] text-black font-bold shadow-md" : ""
                }`
              }
              title={link.name}
              end
