...
// const SingleRecipe = () => {
//   // use useparam to get the id 
//   // use the id to make a request to the end point to fetch your single item
//   // pass the values to the body of return
//   return (
...
const SingleRecipe = () => {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/recipe/${id}`);
  ...
  if (!recipe) return <p className="text-center py-10 text-red-600">Recipe not found.</p>;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <img
        src={recipe.image || "/assets/images/tz.jpg"}
        alt={recipe.title}
        className="w-full h-[400px] object-cover rounded-lg shadow mb-8"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-3 gap-10">
        ...
        <div className="bg-white shadow-md rounded-xl p-6 space-y-4">
          ...
          <div className="flex justify-between items-center border-b pb-2">
            ...
            <span className="text-gray-600">{recipe.timeToCook} min</span>
          </div>
          <div className="flex justify-between items-center border-b pb-2">
            <span className="font-semibold text-gray-700 flex items-center gap-2">
              Difficulty
      ...
      <div className="mt-12 mb-8">
        ...
        <div className="relative pb-[56.25%] h-0 overflow-hidden rounded-lg shadow-lg">
          <iframe
            className="absolute top-0 left-0 w-full h-full"
            src={recipe.videoUrl || "https://www.youtube.com/embed/ONB2HtE7UMA"}
            title="Recipe Video"
            frameBorder="0"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
